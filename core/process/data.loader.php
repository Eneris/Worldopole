	...
	switch ($page) {
		...
		case 'pokemon':
			...
			$req = "SELECT trainer_name, ROUND(SUM(100*(iv_attack+iv_defense+iv_stamina)/45),1) as IV, move_1, move_2, cp,
					...
					ORDER BY $best_order_by $best_direction, cp DESC, trainer_name ASC    # Secondary sort by cp, third by name
					LIMIT 0,25";
...
/////////////
	...
	if ($config->system->recents_filter) {
		...
		$req 		= "SELECT DISTINCT pokemon_id, encounter_id, disappear_time, last_modified, (CONVERT_TZ(disappear_time, '+00:00', '".$time_offset."')) as disappear_time_real, latitude, longitude, individual_attack, individual_defense, individual_stamina, move_1, move_2 FROM pokemon
